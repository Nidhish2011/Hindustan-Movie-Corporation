...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- 3D CARD TILT LOGIC ---
    const tiltElements = document.querySelectorAll('.movie-poster, .news-card, .track');
    ...
    window.setupTrackPlayers = function() {
        ...
        tracks.forEach(track => {
            ...
            if(btn && audio) {
                btn.removeEventListener('click', btn._playerHandler);
                btn._playerHandler = () => {
                ...
                btn.addEventListener('click', btn._playerHandler);
            }
        });
    };

    // Run track player setup once on page load for static tracks.
    ...
    const closeModal = () => {
        ...
        setTimeout(() => { if (trailerFrame) trailerFrame.src = ""; }, 400); // Stop video audio
    };

    document.body.addEventListener('click', (e) => {
        const trigger = e.target.closest('.modal-trigger');
        if (!trigger || !trailerModal || !trailerFrame) return;
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            if (trailerModal && trailerModal.classList.contains('active')) closeModal();
    ...
    // --- HERO VIDEO CAROUSEL ---
    const videoSources = ['', '', '']; // Add your video URLs here
    ...
    function changeVideo() {
        ...
        if (videoSources[currentVideoIndex]) {
